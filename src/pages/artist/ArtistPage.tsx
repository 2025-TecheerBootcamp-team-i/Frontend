...
export default function ArtistPage() {
    ...
    // ✅ URL이 바뀌었는지 감지하기 위한 상태 (네비게이션 시 깜빡임 방지)
    ...
    // ✅ 로딩 중이거나, URL의 ID와 데이터를 불러온 ID가 다르면 로딩 화면 표시
    if (loading || artistId !== fetchedId) {
        return (
    ...
    // ✅ 에러가 났거나 아티스트 데이터가 없을 때도 (사용자 요청) 로딩 화면만 표시
    if (!artist || error) {
        return (
            <div className="w-full min-w-0 px-6 py-5 text-white">
                <button
