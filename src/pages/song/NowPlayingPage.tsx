...
function SimpleWordCloud({
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full min-h-[0px] overflow-hidden"
    />
  );
}
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const animation = particle.animate(
      [
        ...
        {
          transform: `translate(calc(-50% + ${tx}px), calc(-50% + ${ty}px)) scale(${0.5 + Math.random()}) rotate(${rotation}deg)`,
          opacity: 0,
        },
      ],
      {
...
export default function NowPlayingPage() {
  ...
  const currentMusicId =
    (current as { musicId?: number | null } | null)?.musicId ?? null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (Number.isNaN(itunesId)) {
        console.warn(
          `[NowPlayingPage] ID Resolution: Invalid current.id`,
          current.id,
        );
        return;
      }
      ...
      try {
        ...
        if (searchRes.ok) {
          ...
          if (matchedResult?.music_id) {
            ...
            try {
              ...
              if (albumRes.ok) {
                const albumData =
                  (await albumRes.json()) as AlbumDetailResponse;
        ...
        if (combinedSearchRes.ok) {
          const combinedSearchData =
            (await combinedSearchRes.json()) as SearchApiResponse;
        ...
        console.warn(
          `[NowPlayingPage] Could not resolve music_id for: ${current.title}`,
        );
      } catch (err) {
  ...
  const [treemapBaseColor, setTreemapBaseColor] = useState<{
    h: number;
    s: number;
    l: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setLyricsError(
          err instanceof Error ? err.message : '가사를 불러올 수 없습니다.',
        );
        setLyrics([]);
      } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const valenceRaw =
          typeof data.valence === 'string'
            ? parseFloat(data.valence)
            : typeof data.valence === 'number'
              ? data.valence
              : 0;
        const arousalRaw =
          typeof data.arousal === 'string'
            ? parseFloat(data.arousal)
  ...
  const handleDragOver =
    (index: number, isQueue: boolean) => (e: DragEvent<HTMLElement>) => {
      if (!isQueue) return;
      e.preventDefault();
    ...
    <div className="relative h-full w-full text-[#F6F6F6] overflow-hidden">
        ...
        <div className="flex-1 p-4 pb-[100px] overflow-hidden flex flex-col min-h-0">
          ...
          >
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <circle
                          ...
                          strokeDashoffset={
                            2 * Math.PI * 93 * (1 - musicAnalysis.valence / 100)
                          }
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="text-3xl font-black text-[#AFDEE2] tabular-nums">
                          {musicAnalysis.valence}
                        </div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <circle
                          ...
                          strokeDasharray={2 * Math.PI * 93}
                          strokeDashoffset={
                            2 * Math.PI * 93 * (1 - musicAnalysis.arousal / 100)
                          }
                          className="transition-all duration-1000 ease-out"
                        />
                        <defs>
                      ...
                      {/* 중앙 텍스트 - 폰트 사이즈 조절 */}
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="text-3xl font-black text-[#E4524D] tabular-nums">
                          {musicAnalysis.arousal}
                        </div>
                        <div className="text-sm text-white/60 font-medium">
                          %
                        </div>
            ...
            <div className="rounded-[24px] border border-white/10 bg-white/[0.05] backdrop-blur-xl p-4 shadow-xl flex flex-col overflow-hidden relative min-h-0">
              <div className="text-lg font-bold text-white/95 mb-2 tracking-wide z-10 relative">
                유사 음악
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 p-3 flex flex-col justify-end text-left bg-gradient-to-t from-black/80 via-black/20 to-transparent">
                          ...
                          <div className="text-sm text-white/80 truncate drop-shadow-md">
                            {rec.artist_name}
                          </div>
                        </div>
      ...
      >
        <div className="mt-12 h-16 px-4 flex items-center justify-between border-b border-white/10">
          <div className="px-4 text-sm font-bold text-white/90 tracking-wide">
            재생목록
          </div>
          <button
            type="button"
      ...
      <div className="h-full relative">
        ...
        <div className="absolute right-8 top-6 z-20 flex items-center gap-5">
          ...
          <button
            type="button"
            onClick={() =>
              setViewMode((v) => (v === 'cover' ? 'vinyl' : 'cover'))
            }
          ...
          <div className="h-full flex flex-col items-center justify-center px-6 pb-20">
            <div className="w-full max-w-[860px] flex flex-col items-center gap-6">
                    ...
                    >
                      {hasTrack && coverImage ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-white/5">
                          <span className="text-white/20 font-black tracking-tighter text-4xl uppercase">
                            No Cover
                          </span>
                        </div>
                      )}
              ...
              <div className="min-w-0 text-center space-y-2 mt-4">
                <div className="text-3xl font-bold text-white truncate tracking-tight drop-shadow-xl">
                  {hasTrack ? current.title : '재생 중인 곡이 없어요'}
                </div>
                <div className="text-lg font-medium text-white/40 truncate tracking-wide">
              ...
              <button
                ...
                className="mt-8 px-6 py-2.5 rounded-full bg-white/5 border border-white/5 text-sm font-bold text-white/60 hover:text-white hover:bg-white/10 transition-all"
              >
                전체 가사 보기
              </button>
            </div>
      ...
      <div
        className={[
          'absolute inset-0 z-50 transition-all duration-500 flex flex-col',
          lyricsOpen
      ...
      >
        ...
        <div className="relative flex-1 flex flex-col h-full">
          ...
          <div className="h-20 px-10 flex items-center justify-between border-b border-white/5">
            ...
            <button
              onClick={() => setLyricsOpen(false)}
              className="p-3 rounded-full hover:bg-white/10 transition text-white/60 hover:text-white"
            >
              <MdClose size={28} />
            </button>
          </div>
            ...
            {lyricsLoading ? (
              ...
              <div className="max-w-3xl mx-auto space-y-10 text-center">
                {lyrics.map((line, i) => (
                  <div
                    key={`${line.t}-${i}`}
                    className="text-3xl font-bold text-white/80 hover:text-white transition-colors cursor-default leading-relaxed"
                  >
                    {line.text}
                  </div>
                ))}
