// src/pages/chart/ChartDaily.tsx
...
export default function ChartDaily() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        for (const item of data.items) nextPrev[String(item.musicId)] = item.rank;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[ChartDaily] 플레이리스트 목록 불러오기 실패:", e);
        if (cancelled) return;
        setAddTargets([]);
  ...
  const addSelectedToPlaylist = useCallback(
    async (playlistId: string) => {
      ...
      try {
        ...
        console.error("[ChartDaily] 플레이리스트 담기 실패:", e);
  ...
  const addSelectedToLiked = useCallback(async () => {
    ...
    try {
      ...
      console.error("[ChartDaily] 좋아요 실패:", e);
  ...
  return (
    <section className="whitespace-nowrap rounded-[40px] bg-white/[0.05] backdrop-blur-2xl border border-white/10 overflow-hidden">
      ...
      {/* ✅ 재생 방식 선택 모달 */}
      <PlayModeConfirmModal open={playConfirmOpen} pending={pendingPlay} onClose={closePlayConfirm} onChoose={runPendingPlay} />
    </section>
  );
}
