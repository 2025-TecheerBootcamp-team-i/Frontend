...
function HomePage() {
  ...
  /* Black Hole Easter Egg Global State */
  const { isActive: isBlackHoleActive } = useEasterEgg();
  ...
  const getDebrisVariant = (delay: number) => ({
    ...
    suck: {
      scale: [1, 0.9, 0],
      x: [0, (Math.random() - 0.5) * 100, 0], // Random jitter before sucking
      y: [0, (Math.random() - 0.5) * 100, 0],
      opacity: [1, 1, 0],
      rotate: [0, (Math.random() - 0.5) * 30, (Math.random() > 0.5 ? 1 : -1) * 720], // Random rotation direction
      transition: {
        duration: 3 + Math.random() * 1.5, // Random duration between 3s and 4.5s
  ...
  return (
    <>
      {/* Main Container (No global animation) */}
      <div className="w-full overflow-x-auto">
        <div className="min-w-[1280px] relative">



          {/* 1. Artist List Section */}
          <motion.div
            variants={getDebrisVariant(0)}
            initial="initial"
          ...
          {/* 2. Grid Section (Left: Banner/AI + Right: Chart) */}
          <section className="mb-6 grid grid-cols-12 gap-6 h-[560px]">
            {/* Left Column Debris */}
            <motion.div
              className="col-span-5 flex flex-col gap-6 h-full"
            ...
            {/* Right Column Debris (Chart) */}
            <motion.div
              className="col-span-7 h-full"
              variants={getDebrisVariant(0.4)}
              initial="initial"
              animate={isBlackHoleActive ? "suck" : "initial"}
            >
              <CompactChart
          ...
          {/* 4. Genre Station Debris */}
          <motion.div
            variants={getDebrisVariant(0.8)}
            initial="initial"
            animate={isBlackHoleActive ? "suck" : "initial"}
          >
            <GenreStationSection />
