        ...
        <div className="flex-1 overflow-y-auto hover-scrollbar px-2 pb-4">
          <div className="flex flex-col">
            {filteredPlaylists.map((playlist) => {
              // 좋아요 표시된 곡(시스템 플레이리스트) 확인
              // PlaylistContext에서 이미 likes/system 로직 처리됨
              // 여기서는 렌더링만 신경 쓰면 됨
              const isSystemLiked = playlist.title === "내 좋아요 목록" || playlist.visibility === "system";

              return (
                <div
                  key={playlist.id}
                  onClick={() => handleNavigate(playlist.id, isSystemLiked)}
                  className="
                      group
